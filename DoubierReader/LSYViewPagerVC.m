    ...
    if ([self.dataSource respondsToSelector:@selector(numberOfViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<numberOfViewController; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexOfViewControllers:)]) {
                ...
                [button addTarget:self action:@selector(p_titleButtonClick:) forControlEvents:UIControlEventTouchUpInside]; //设置动作
                
                button.frame = CGRectMake(oldRect.origin.x + oldRect.size.width , 0, 75, [self.dataSource respondsToSelector:@selector(heightForTitleOfViewPager:)]?[self.dataSource heightForTitleOfViewPager:self]:0);
...
//    [_pageViewController addChildViewController:@[arrayOfViewController[index]]];
//    [_pageViewController.view setAutoresizesSubviews:NO];
